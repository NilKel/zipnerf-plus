...
class PotentialTriMipEncoding(nn.Module):
    def __init__(
        self,
        n_levels: int,
        plane_size: int,
        ...
        super(PotentialTriMipEncoding, self).__init__()
        self.n_levels = n_levels
        self.plane_size = plane_size
        self.feature_dim = feature_dim
        self.vector_dim = 3
        ...
        self.register_parameter(
            "fm",
            nn.Parameter(torch.zeros(3, plane_size, plane_size, feature_dim * self.vector_dim)),
        )
        self.init_parameters()
        self.dim_out = (
            self.feature_dim * 3 * self.vector_dim + 3 if include_xyz else self.feature_dim * 3 * self.vector_dim
        )
    ...
    def forward(self, x, level):
        ...
        for i, coords in enumerate(plane_coords):
            ...
            # Get plane features: [H, W, C*3] -> [C*3, H, W] -> [1, C*3, H, W]
            plane = self.fm[i].permute(2, 0, 1).unsqueeze(0)
            ...
            # Reshape to [N, C*3]
            plane_features = plane_features.squeeze(0).squeeze(1).permute(1, 0)
            features.append(plane_features)
        
        # Concatenate features from all three planes
        enc = torch.cat(features, dim=-1)  # [N, 3*C*3]
